...
Никогда не публикуйте реальный `awg0.conf`, private/preshared keys, VLESS-ссылки, токены, локальные DNS-маршруты, логи или кэши. `.gitignore` исключает эти категории, но перед коммитом проверьте индекс:
...
Для автоматизации и других агентов есть подробные ограничения и команды в [AGENT.md](AGENT.md).
